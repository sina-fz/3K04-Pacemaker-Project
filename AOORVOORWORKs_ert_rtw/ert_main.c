...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AOORVOORWORKs_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    AOORVOORWORKs_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(AOORVOORWORKs_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AOORVOORWORKs_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(AOORVOORWORKs_M, 0);

  /* Initialize model */
  AOORVOORWORKs_initialize();
